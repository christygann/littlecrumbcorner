...
// All text sizes are +2px vs the sign-ups dashboard equivalents
...
function NewOrderTab({ onOrderPlaced }) {
  ...
  const updateCartQty = (name, newQty) => {
    ...
    setCart(prev =>
      prev.map(c => c.name === name ? { ...c, qty: q, subtotal: parseFloat((c.price * q).toFixed(2)) } : c)
    )
  }
  ...
  return (
    <div className="flex flex-col lg:flex-row lg:justify-between gap-8 items-start">
      ...
      {/* Right — order summary, pushed to far right on large screens */}
      <div className="w-full lg:w-80 lg:shrink-0 lg:sticky lg:top-6 lg:self-start">
        <div className="bg-white rounded-[0.9rem] shadow-[0_2px_14px_rgba(107,76,59,0.05)] overflow-hidden">
          ...
          {cart.length === 0 ? (
            <div className="px-5 py-10 text-center font-display italic text-[18px] text-cocoa/30">
              no items yet
            </div>
          ) : (
            <>
              {cart.map(c => (
                <div key={c.name} className="flex items-center gap-3 px-5 py-3.5 border-b border-cocoa/[0.04] last:border-none hover:bg-cream/60 transition-colors">
                  ...
                  <button
                    ...
                    className="text-[13px] text-deep-rose/50 hover:text-deep-rose transition-colors cursor-pointer shrink-0"
                  >✕</button>
                </div>
              ))}
...
function AllOrdersTab({ orders, loading, onDelete }) {
    ...
    <div>
      ...
      {orders.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {orders.map(o => (
            <div key={o.id} className="bg-white rounded-[0.9rem] shadow-[0_2px_14px_rgba(107,76,59,0.05)] overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-cocoa/[0.06] flex-wrap gap-2">
                <div className="flex items-baseline gap-3 min-w-0">
                  <span className="text-[17px] text-espresso font-display font-light truncate">{o.customer_name}</span>
                  <span className="text-[13px] text-cocoa/40 whitespace-nowrap shrink-0">{fmtDate(o.created_at)}</span>
                </div>
  ...
  return (
    <div className="flex-1 p-4 sm:p-10 max-w-[1100px] w-full mx-auto">
      <div className="flex items-end justify-between mb-6 flex-wrap gap-4">
        <div>
          <h2 className="font-display font-light text-[32px] text-espresso">orders</h2>
          <div className="text-[15px] text-cocoa/60 mt-1">27 May 2026</div>
        </div>
        {tab !== 'new' && (
      ...
      <div className="flex gap-1 mb-7 bg-white rounded-full p-1 w-fit shadow-[0_2px_14px_rgba(107,76,59,0.05)]">
        {TABS.map(t => (
          <button
            key={t.key}
      ...
      {tab === 'orders' && <AllOrdersTab orders={orders} loading={loading} onDelete={handleDelete} />}
